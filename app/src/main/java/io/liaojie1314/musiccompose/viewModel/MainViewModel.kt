package io.liaojie1314.musiccompose.viewModel
...
import io.liaojie1314.musiccompose.utils.AppUtils
...
class MainViewModel : ViewModel() {
    ...
    private fun loadLocalMusicData() {
        ...
        when {
            ...
            !cursor.moveToFirst() -> {
                ToastUtils.showShortMsg(AppUtils.context, "设备上没有歌曲")
            }
            else -> {
    ...
    fun playLastMusic(){
        if(currentPlayPosition==0||currentPlayPosition==-1){
            ToastUtils.showShortMsg(AppUtils.context, "已经是第一首了，没有上一曲！")
        }else{
            //获取上一首歌曲
    ...
    fun playCurrentMusic(){
        if (currentPlayPosition == -1) {
            ToastUtils.showShortMsg(AppUtils.context, "请选择想要播放的音乐")
        } else {
    ...
    fun playNextMusic(){
        if(currentPlayPosition==mData.size-1||currentPlayPosition==mData.size){
            ToastUtils.showShortMsg(AppUtils.context, "已经是最后一首了，没有下一曲！")
        }else{
            //获取上一首歌曲
